...
The experimental pipeline used three dataset roles. The first was a labeled source-domain oil palm FFB dataset based mainly on ScienceDB/OILPALM-style video-frame imagery. This dataset was used to train the source-only supervised baseline and to construct a scene-disjoint split. The second was an external-domain preprocessed FFB dataset, which was used as the main target-domain dataset for cross-domain evaluation and adaptation. The third was an outdoor oil palm ripeness dataset used only for exploratory third-domain zero-shot image-level evaluation.

The unified class scheme contained six maturity or condition classes:
\[
...
This design follows the general principle of consistency regularization in semi-supervised learning: a reliable object prediction should remain stable under label-preserving image transformations. In this work, consistency was used as a filtering criterion rather than a fully online teacher-student loss, making the method easier to reproduce with standard YOLOv8 training tools.
...
The source-only YOLOv8n model achieved scene-disjoint source-domain mAP@0.5 of 0.5917 and mAP@0.5:0.95 of 0.5018 across three seeds. However, when evaluated on the locked external preprocessed FFB dataset, performance dropped to mAP@0.5 of 0.0372 and mAP@0.5:0.95 of 0.0130. This confirms severe cross-domain degradation. Therefore, the source-only result should be interpreted as evidence of domain shift rather than as the main baseline for claiming method improvement.
...
The precision-first strict SSOD model achieved scene-disjoint mAP@0.5 of 0.6373 and mAP@0.5:0.95 of 0.5408. Under calibrated external-domain evaluation, it achieved mAP@0.5 of 0.7240, mAP@0.5:0.95 of 0.4863, precision of 0.8753, recall of 0.5364, and F1-score of 0.6321. Compared with the class-balanced adaptation baseline, strict SSOD mainly improved the high-precision operating point rather than producing a large headline mAP improvement.
...
The proposed framework has several limitations. The external preprocessed FFB dataset lacks empty and overripe categories, requiring masked evaluation. Although this is methodologically necessary, it means the reported external-domain result does not cover the full six-class maturity scheme. The third-domain outdoor dataset was used only for image-level evaluation and had limited class balance, so it cannot replace a large object-detection external test set. Finally, the SSOD stage used offline pseudo-label filtering rather than a fully online EMA teacher-student training loop. This choice improves reproducibility and engineering simplicity but may limit the maximum performance gain.
...
The source and external datasets used in this study are derived from publicly available or locally prepared oil palm FFB datasets. The exact train, validation, test, and locked external split files should be released with the manuscript where dataset licenses permit. TODO: add a public repository link or Zenodo archive for processed split manifests, training scripts, and evaluation reports before submission.

\section*{Code Availability Statement}

The implementation was developed in Python using the Ultralytics YOLOv8 API. The pipeline includes dataset preparation, scene-disjoint splitting, duplicate checking, pseudo-label generation, quality scoring, class-wise threshold calibration, and structured JSON/CSV reporting. TODO: add GitHub or Zenodo code archive link before submission.
...
This research received no external funding. TODO: revise if institutional or project funding must be acknowledged.

\section*{Acknowledgements}

The author thanks the maintainers of the public oil palm FFB datasets and open-source computer vision libraries used in this work. TODO: add any institutional acknowledgement required by Xiamen University Malaysia.
...
\bibitem{coco}
T.-Y. Lin et al., ``Microsoft COCO: Common objects in context,'' in \textit{European Conference on Computer Vision}, 2014, pp. 740--755.
...
\bibitem{fruit_detection_review}
...
\bibitem{zenodo_ffb}
Outdoor Oil Palm Fruit Ripeness Dataset, Zenodo, 2024. doi: 10.5281/zenodo.11114885.

\bibitem{ultralytics_docs}
Ultralytics, ``YOLO documentation,'' 2024. [Online]. Available: \url{https://docs.ultralytics.com}
