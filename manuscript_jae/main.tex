...
Xiamen University Malaysia, Jalan Sunsuria, Bandar Sunsuria,\\
43900 Sepang, Selangor Darul Ehsan, Malaysia\\
*Corresponding author: WU YUNING, eee2309312@xmu.edu.my
}
...
The experimental pipeline used three dataset roles, summarized in table~\ref{tab:dataset_roles}. The first was a labeled source-domain oil palm FFB dataset based mainly on ScienceDB/OILPALM-style video-frame imagery \citep{suharjito2023dataset}. This dataset was used to train the source-only supervised baseline and to construct a scene-disjoint split. The second was an external-domain preprocessed FFB dataset, which was used as the main target-domain dataset for cross-domain evaluation and adaptation. The third was an outdoor oil palm ripeness dataset used only for exploratory third-domain zero-shot image-level evaluation \citep{omar2024outdoor}.

The complete workflow is summarized in figure~\ref{fig:framework_workflow}. The protocol separates source-domain training, external-domain adaptation, weakly guided pseudo-label filtering, locked external testing, and third-domain evaluation to avoid mixing training evidence with final evaluation evidence. The model-level flow from source-only training to quality-controlled strict SSOD is shown in figure~\ref{fig:model_flow}.
...
Let \(x\) be an input image, \(T_k(\cdot)\) be the \(k\)-th label-preserving augmentation, and \(T_k^{-1}(\cdot)\) be the inverse mapping from the augmented coordinate system back to the original image. A teacher prediction on the original image is denoted by \(p_i=(b_i,c_i,s_i)\), where \(b_i\) is the bounding box, \(c_i\) is the predicted class, and \(s_i\) is the confidence score. The mapped prediction from an augmented view is denoted by \(\tilde{p}_{ik}=(T_k^{-1}(\tilde{b}_{ik}),\tilde{c}_{ik},\tilde{s}_{ik})\). A candidate pseudo-label was considered consistent when
\[
\mathbb{I}_{ik} =
\mathbf{1}\left[
c_i=\tilde{c}_{ik}
\ \land\
\operatorname{IoU}\left(b_i,T_k^{-1}(\tilde{b}_{ik})\right)\ge \gamma
\right],
\]
where \(\gamma\) is the IoU consistency threshold. The multi-view consistency score was then computed as
\[
Q_m(p_i)=\frac{1}{K}\sum_{k=1}^{K}\mathbb{I}_{ik},
\]
and candidates with insufficient view agreement were rejected before training.

This design follows the general principle of consistency regularization in semi-supervised learning: a reliable object prediction should remain stable under label-preserving image transformations \citep{tarvainen2017mean,sohn2020fixmatch}. In this work, consistency was used as a filtering criterion rather than a fully online teacher-student loss, making the method easier to reproduce with standard YOLOv8 training tools. The approach is related to pseudo-label and consistency-based SSOD methods \citep{sohn2020stac,liu2021unbiased,xu2021softteacher,li2022pseco}, but it is implemented as an offline quality-control layer for an agricultural engineering workflow.
...
For a box with width \(w_b\), height \(h_b\), image width \(W\), and image height \(H\), the normalized box area and aspect ratio were defined as
\[
a_b=\frac{w_bh_b}{WH}, \qquad r_b=\frac{w_b}{h_b}.
\]
The geometry score was represented as an indicator-style plausibility term,
\[
Q_g=\mathbf{1}\left[a_{\min}\le a_b\le a_{\max}\right]
\mathbf{1}\left[r_{\min}\le r_b\le r_{\max}\right],
\]
where the limits describe expected FFB box sizes and aspect ratios after resizing. Edge contact was penalized using the minimum normalized distance from the box to the image boundary,
\[
d_e=\min\left(\frac{x_1}{W},\frac{y_1}{H},\frac{W-x_2}{W},\frac{H-y_2}{H}\right), \qquad
Q_e=\min\left(1,\frac{d_e}{\delta}\right),
\]
where \((x_1,y_1,x_2,y_2)\) are box coordinates and \(\delta\) is the boundary-margin tolerance. Folder-level weak consistency was defined as
\[
Q_f=\mathbf{1}\left[c_i\in \mathcal{C}_{folder}(x)\right],
\]
where \(\mathcal{C}_{folder}(x)\) is the set of image-level weak classes inferred from the folder name when available. If no folder prior was available, this term was treated as neutral during scoring rather than as negative evidence.
...
For class \(c\), the adaptive threshold was written as
\[
\tau_c=\operatorname{clip}\left(\tau_0+\lambda(1-\mathrm{AP}_c),\tau_{\min},\tau_{\max}\right),
\]
where \(\tau_0\) is the base confidence threshold, \(\mathrm{AP}_c\) is the class-wise validation average precision, \(\lambda\) controls the penalty for difficult classes, and \(\tau_{\min}\) and \(\tau_{\max}\) bound the threshold range. A pseudo-label candidate for class \(c_i\) was accepted only if
\[
s_i\ge \tau_{c_i}, \qquad Q(p_i)\ge \eta, \qquad Q_m(p_i)\ge \mu,
\]
where \(\eta\) is the minimum composite quality score and \(\mu\) is the minimum multi-view consistency score.

To reduce class imbalance during self-training, accepted pseudo-labeled images were selected under a per-class quota:
\[
|\mathcal{P}^{(t)}_c|\le N_c^{\max},
\]
where \(\mathcal{P}^{(t)}_c\) is the set of pseudo-labeled samples accepted for class \(c\) at self-training iteration \(t\), and \(N_c^{\max}\) is the maximum number of pseudo-labeled images allowed for that class.
...
The primary detection metrics were precision, recall, F1-score, mAP@0.5, and mAP@0.5:0.95. Precision, recall, and F1-score were computed as
\[
...
\]
The third-domain experiment was reported separately as image-level top-1 accuracy and macro F1-score, and should not be compared directly with detection mAP.
...
The precision-first strict SSOD model achieved scene-disjoint mAP@0.5 of 0.6373 and mAP@0.5:0.95 of 0.5408. Under calibrated external-domain evaluation, it achieved mAP@0.5 of 0.7240, mAP@0.5:0.95 of 0.4863, precision of 0.8753, recall of 0.5364, and F1-score of 0.6321 (table~\ref{tab:strict_ssod}; figure~\ref{fig:external_map}). Compared with the class-balanced adaptation baseline, strict SSOD mainly improved the high-precision operating point rather than producing a large headline mAP improvement. The calibration curve in figure~\ref{fig:calibration} illustrates this precision-recall trade-off.
...
\section*{Declaration of Generative Artificial Intelligence and Artificial Intelligence-Assisted Technologies in the Writing Process}
...
\bibitem[Bargoti and Underwood(2017)]{bargoti2017fruit}
Bargoti S, Underwood J, 2017. Deep fruit detection in orchards. Proc. IEEE Int. Conf. Robot. Autom., pp. 3626--33, doi: 10.1109/ICRA.2017.7989417.

\bibitem[Jocher et al.(2023)]{jocher2023yolov8}
Jocher G, Chaurasia A, Qiu J, 2023. Ultralytics YOLOv8. Available from: \url{https://github.com/ultralytics/ultralytics}

\bibitem[Kamilaris and Prenafeta-Boldu(2018)]{kamilaris2018survey}
Kamilaris A, Prenafeta-Boldu FX, 2018. Deep learning in agriculture: a survey. \textit{Comput. Electron. Agric.} 147:70--90, doi: 10.1016/j.compag.2018.02.016.

\bibitem[Lai et al.(2022)]{lai2022yolov4}
Lai JW, Ramli HR, Ismail LI, Hasan WZW, 2022. Real-time detection of ripe oil palm fresh fruit bunch based on YOLOv4. \textit{IEEE Access} 10:95763--70, doi: 10.1109/ACCESS.2022.3204762.

\bibitem[Li et al.(2022)]{li2022pseco}
Li J et al., 2022. PseCo: pseudo labeling and consistency training for semi-supervised object detection. \textit{Eur. Conf. Comput. Vis.}, doi: 10.1007/978-3-031-20077-9\_27.

\bibitem[Liu et al.(2021)]{liu2021unbiased}
Liu YC et al., 2021. Unbiased teacher for semi-supervised object detection. \textit{Int. Conf. Learn. Represent.}, doi: 10.48550/arXiv.2102.09480.

\bibitem[Mansour et al.(2022)]{mansour2022object}
Mansour MYMA, Dambul KD, Choo KY, 2022. Object detection algorithms for ripeness classification of oil palm fresh fruit bunch. \textit{Int. J. Technol.} 13:1326--35, doi: 10.14716/ijtech.v13i6.5932.

\bibitem[Omar et al.(2024)]{omar2024outdoor}
Omar Z, Abdul Majeed APP, Rosbi M, Ghazalli SA, Selamat H, 2024. Outdoor oil palm fruit ripeness dataset. \textit{Data Brief} 55:110667, doi: 10.1016/j.dib.2024.110667.

\bibitem[Redmon et al.(2016)]{redmon2016yolo}
Redmon J, Divvala S, Girshick R, Farhadi A, 2016. You only look once: unified, real-time object detection. Proc. IEEE Conf. Comput. Vis. Pattern Recognit., pp. 779--88, doi: 10.1109/CVPR.2016.91.

\bibitem[Sohn et al.(2020a)]{sohn2020fixmatch}
Sohn K et al., 2020a. FixMatch: simplifying semi-supervised learning with consistency and confidence. \textit{Adv. Neural Inf. Process. Syst.}, doi: 10.48550/arXiv.2001.07685.

\bibitem[Sohn et al.(2020b)]{sohn2020stac}
Sohn K et al., 2020b. A simple semi-supervised learning framework for object detection. \textit{arXiv preprint arXiv:2005.04757}, doi: 10.48550/arXiv.2005.04757.

\bibitem[Suharjito et al.(2021)]{suharjito2021mobile}
Suharjito, Elwirehardja GN, Prayoga JS, 2021. Oil palm fresh fruit bunch ripeness classification on mobile devices using deep learning approaches. \textit{Comput. Electron. Agric.} 188:106359, doi: 10.1016/j.compag.2021.106359.

\bibitem[Suharjito et al.(2023)]{suharjito2023dataset}
Suharjito, Junior FA, Koeswandy YP, Debi, Nurhayati PW, Asrol M, Marimin, 2023. Annotated datasets of oil palm fruit bunch piles for ripeness grading using deep learning. \textit{Sci. Data} 10:72, doi: 10.1038/s41597-023-01958-x.

\bibitem[Tarvainen and Valpola(2017)]{tarvainen2017mean}
Tarvainen A, Valpola H, 2017. Mean teachers are better role models: weight-averaged consistency targets improve semi-supervised deep learning results. \textit{Adv. Neural Inf. Process. Syst.}, doi: 10.48550/arXiv.1703.01780.

\bibitem[Xu et al.(2021)]{xu2021softteacher}
Xu M et al., 2021. End-to-end semi-supervised object detection with soft teacher. Proc. IEEE/CVF Int. Conf. Comput. Vis., doi: 10.48550/arXiv.2106.09018.
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.26\textwidth}@{}}
\toprule
...
\midrule
Source-only YOLOv8n & $0.5917 \pm 0.0339$ & $0.5018 \pm 0.0343$ & $0.0372 \pm 0.0221$ & $0.0130 \pm 0.0075$ \\
Model2 Balanced & $0.6459 \pm 0.0190$ & $0.5484 \pm 0.0149$ & $0.6973 \pm 0.0298$ & $0.4758 \pm 0.0175$ \\
Stage-1 SSOD teacher & $0.6141 \pm 0.0340$ & $0.5125 \pm 0.0183$ & $0.6619 \pm 0.0283$ & $0.4462 \pm 0.0309$ \\
Stage-2 refined & $0.5878 \pm 0.0436$ & $0.4900 \pm 0.0395$ & $0.6754 \pm 0.0430$ & $0.4645 \pm 0.0331$ \\
\bottomrule
\end{tabular}
...
\midrule
Source-only YOLOv8n & $0.1640 \pm 0.0747$ & $0.0933 \pm 0.0530$ & $0.0735 \pm 0.0308$ \\
Model2 Balanced & $0.6126 \pm 0.0323$ & $0.7527 \pm 0.0121$ & $0.6442 \pm 0.0332$ \\
Stage-1 SSOD teacher & $0.6163 \pm 0.0468$ & $0.7411 \pm 0.0473$ & $0.6390 \pm 0.0174$ \\
Stage-2 refined & $0.6294 \pm 0.0402$ & $0.7427 \pm 0.0338$ & $0.6743 \pm 0.0386$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/fig_framework_workflow.png}
\caption{Overview of the proposed quality-controlled cross-domain SSOD workflow. Data sources, protocol controls, model stages, and evaluation targets are separated to preserve the distinction between source-only transfer, target-domain adaptation, and precision-first strict SSOD.}
\label{fig:framework_workflow}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/fig_model_flow.png}
\caption{Model-level flow of the proposed quality-controlled SSOD pipeline. Model 0 is used to demonstrate source-to-external domain shift, Model 2 Balanced is used as the main fair adaptation baseline, and the strict SSOD student is trained only after multi-view pseudo-label consistency, composite quality scoring, dynamic class-wise thresholds, and balanced pseudo-label selection.}
